...
//                        TimeofQuiz = 1;
                ...
                int questionNumber = Integer.parseInt((String) request.getParameter("NUMBER"));
                ...
                request.setAttribute("CURRENTQUESTION", list.get(questionNumber).getId());
                ...
                String temp = request.getParameter("answerGroup");
                if (temp != null) {
                    int ansID = Integer.parseInt(temp);
                ArrayList<AnswerDTO> ans = map.get(questionid);
                boolean correct = true;
                for (AnswerDTO an : ans) {
                    if (an.getId() == ansID) {
                        an.setCheck(true);
                        correct = an.isCorrect();
                    }
                }
                AnswerRecords records = (AnswerRecords) session.getAttribute("RECORDS");
                records.addAnswer(questionid, correct);
                session.setAttribute("ANSWERMAP", map);
                }
              
                String timeleft = request.getParameter("timeleft");
                double time = Double.parseDouble(timeleft);
                session.setAttribute("TIMEQUIZ", time/60000);
